...
type AddWatchProp<T> = T extends Map<infer K, infer V> ? WatchProp<Map<K, AddWatchProp<V>>> : T extends WeakMap<infer K, infer V> ? WatchProp<WeakMap<K, AddWatchProp<V>>> : T extends Set<infer U> ? WatchProp<Set<AddWatchProp<U>>> : T extends WeakSet<infer U> ? WatchProp<WeakSet<AddWatchProp<U>>> : T extends ObjectType ? WatchProp<{
...
declare function computed<T>(getter: ComputedGetter<T>, options?: {
    triggerAnyway?: boolean;
}): ComputedImpl<T>;
declare function useComputed<T>(options: ComputedOptions<T>): ComputedImpl<T>;
declare function useComputed<T>(getter: ComputedGetter<T>, options?: {
    triggerAnyway?: boolean;
}): ComputedImpl<T>;
...
declare class Component<P = KVObject, S = KVObject, SS = any, C = any> {
    ...
    setState<K extends keyof S>(state: ((prevState: Readonly<S>, props: Readonly<P>) => Pick<S, K> | S | null) | Pick<S, K> | S | null, callback?: Callback$1): void;
}
/**
 * 支持PureComponent
 */
...
declare namespace Event {
  export type { Event_DomAnimationEvent as DomAnimationEvent, Event_DomClipboardEvent as DomClipboardEvent, Event_DomCompositionEvent as DomCompositionEvent, Event_DomDragEvent as DomDragEvent, Event_DomFocusEvent as DomFocusEvent, Event_DomKeyboardEvent as DomKeyboardEvent, Event_DomMouseEvent as DomMouseEvent, Event_DomTouchEvent as DomTouchEvent, Event_DomPointerEvent as DomPointerEvent, Event_DomTransitionEvent as DomTransitionEvent, Event_DomUIEvent as DomUIEvent, Event_DomWheelEvent as DomWheelEvent };
}
...
interface ChildrenType {
    map<T, C>(children: C | Array<C>, fn: (child: C, index: number) => T, context?: any): C extends null | undefined ? C : Array<Exclude<T, boolean | null | undefined>>;
...
} ? string extends R ? PropsOmitRef<P> & {
...
type ComponentPropsWithRef<T extends any> = T extends new (props: infer P) => Component<any, any> ? PropsOmitRef<P> & RefAttributes<InstanceType<T>> : PropsWithRef<any>;
...
type ModifierKey = 'Alt' | 'AltGraph' | 'CapsLock' | 'Control' | 'Fn' | 'FnLock' | 'Meta' | 'NumLock' | 'ScrollLock' | 'Shift' | 'Symbol' | 'SymbolLock';
...
declare function createElement(type: any, setting: any, ...children: any[]): {
    [x: string]: any;
    vtype: number;
    src: any;
    type: any;
...
declare function cloneElement(element: any, setting: any, ...children: any[]): {
    [x: string]: any;
    vtype: number;
    src: any;
    type: any;
...
declare function lazy<T extends ComponentType<any>>(promiseCtor: () => Promise<{
    default: T;
}>): LazyComponent<T>;

declare function forwardRef<T, P = any>(render: ForwardRefRenderFunc<T, P>): ExoticComponent<PropsOmitRef<P>> & RefAttributes<T>;

declare function memo<Props extends Record<string, any>>(type: FunctionComponent<Props>, compare?: (oldProps: Readonly<Props>, newProps: Readonly<Props>) => boolean): ExoticComponent<Props>;
declare function memo<T extends ComponentType<any>>(type: T, compare?: (oldProps: any, newProps: any) => boolean): MemoComponent<T>;
...
declare function useReducer<R extends ReducerVoid<any>, I>(reducer: R, initialArg: I, init: (arg: I) => ReducerVoid<R>): [ReducerStateVoid<R>, DispatchVoid];
declare function useReducer<R extends ReducerVoid<any>>(reducer: R, initialArg: ReducerStateVoid<R>, init?: undefined): [ReducerStateVoid<R>, DispatchVoid];
declare function useReducer<R extends Reducer$1<any, any>, I>(reducer: R, initialArg: I & ReducerState<R>, init?: (arg: I & ReducerState<R>) => ReducerState<R>): [ReducerState<R>, Dispatch$1<ReducerAction<R>>];
declare function useReducer<R extends Reducer$1<any, any>, I>(reducer: R, initialArg: I, init?: (arg: I) => ReducerState<R>): [ReducerState<R>, Dispatch$1<ReducerAction<R>>];
declare function useReducer<R extends Reducer$1<any, any>>(reducer: R, initialArg: ReducerState<R>, init?: undefined): [ReducerState<R>, Dispatch$1<ReducerAction<R>>];
...
declare function useImperativeHandle<T, R extends T>(ref: RefObject<T> | RefCallBack<T> | null | undefined, create: () => R, deps?: DependencyList): void;
...
type Action<T extends ActionFunction<any>, S extends Record<string, unknown>> = (this: StoreObj<S, any, any>, ...args: RemoveFirstFromTuple<Parameters<T>>) => ReturnType<T>;
type StoreComputed<S extends Record<string, unknown>, C extends UserComputedValues<S>> = {
    [K in keyof C]: ReturnType<C[K]>;
};
type StoreObj<S extends Record<string, unknown> = Record<string, unknown>, A extends UserActions<S> = UserActions<S>, C extends UserComputedValues<S> = UserComputedValues<S>> = {
    $s: AddWatchProp<S>;
...
type AsyncAction<T extends ActionFunction<any>, S extends Record<string, unknown>> = (this: StoreObj<S, any, any>, ...args: RemoveFirstFromTuple<Parameters<T>>) => Promise<ReturnType<T>>;
...
declare function createStore$1<S extends Record<string, any>, A extends UserActions<S>, C extends UserComputedValues<S>>(config: StoreConfig<S, A, C>): () => StoreObj<S, A, C>;
declare function useStore$1<S extends Record<string, unknown>, A extends UserActions<S>, C extends UserComputedValues<S>>(id: string): StoreObj<S, A, C>;
...
declare function shallowRef<T>(value: T): RefType extends T ? (T extends RefType ? IfAny<T, ShallowRef<T>, T> : ShallowRef<T>) : ShallowRef<T>;
declare function shallowRef<T = any>(): ShallowRef<T | undefined>;
declare function toRef<T>(value: T): T extends () => infer R ? Readonly<RefType<R>> : T extends RefType ? T : RefType<UnwrapRef<T>>;
declare function toRef<T extends Record<string, any>, K extends keyof T>(object: T, key: K): ToRef<T[K]>;
declare function toRef<T extends Record<string, any>, K extends keyof T>(object: T, key: K, defaultValue: T[K]): ToRef<Exclude<T[K], undefined>>;
...
type ReactiveRet<T> = T extends FnType | BaseTypes | RefType ? T : T extends Map<infer K, infer V> ? Map<K, ReactiveRet<V>> & UnwrapRef<Omit<T, keyof Map<any, any>>> : T extends WeakMap<infer K, infer V> ? WeakMap<K, ReactiveRet<V>> & UnwrapRef<Omit<T, keyof WeakMap<any, any>>> : T extends Set<infer V> ? Set<ReactiveRet<V>> & UnwrapRef<Omit<T, keyof Set<any>>> : T extends WeakSet<infer V> ? WeakSet<ReactiveRet<V>> & UnwrapRef<Omit<T, keyof WeakSet<any>>> : T extends ReadonlyArray<any> ? {
...
declare function Provider({ store, context, children, }: {
    store: ReduxStoreHandler;
    context?: Context;
    children?: any[];
}): {
...
type MergePropsP<StateProps, DispatchProps, OwnProps, MergedProps> = (stateProps: StateProps, dispatchProps: DispatchProps, ownProps: OwnProps) => MergedProps;
...
declare function connect<StateProps, DispatchProps, OwnProps, MergedProps>(mapStateToProps?: MapStateToPropsP<StateProps, OwnProps>, mapDispatchToProps?: MapDispatchToPropsP<DispatchProps, OwnProps>, mergeProps?: MergePropsP<StateProps, DispatchProps, OwnProps, MergedProps>, options?: ConnectOption<any, StateProps, OwnProps>): Connector<OwnProps, MergedProps>;
...
type ReduxMiddleware = (store: ReduxStoreHandler, extraArgument?: any) => (next: (action: ReduxAction) => any) => (action: ReduxAction | ((dispatch: (action: ReduxAction) => void, store: ReduxStoreHandler, extraArgument?: any) => any)) => ReduxStoreHandler;
...
declare namespace reduxAdapter {
  export { type reduxAdapter_ReduxStoreHandler as ReduxStoreHandler, type reduxAdapter_ReduxAction as ReduxAction, type reduxAdapter_ReduxMiddleware as ReduxMiddleware, reduxAdapter_createStore as createStore, reduxAdapter_combineReducers as combineReducers, reduxAdapter_applyMiddleware as applyMiddleware, type reduxAdapter_BoundActionCreator as BoundActionCreator, type reduxAdapter_Dispatch as Dispatch, reduxAdapter_bindActionCreators as bindActionCreators, reduxAdapter_compose as compose, reduxAdapter_batch as batch, reduxAdapter_thunk as thunk, reduxAdapter_Provider as Provider, reduxAdapter_useSelector as useSelector, reduxAdapter_useStore as useStore, reduxAdapter_useDispatch as useDispatch, reduxAdapter_connect as connect, reduxAdapter_createSelectorHook as createSelectorHook, reduxAdapter_createDispatchHook as createDispatchHook };
}
...
declare function createPortal(children: InulaNode, realNode: Element | DocumentFragment, key?: null | string): InulaPortal;
...
export { type ActionFunction, type AddWatchProp, type AnimationEvent$1 as AnimationEvent, type AnimationEventHandler, type AsyncAction, type BasicStateAction, type CSSProperties, type ChangeEvent, type ChangeEventHandler, Children, type ChildrenType, type ClipboardEvent$1 as ClipboardEvent, type ClipboardEventHandler, type CollectionStringTypes, type CollectionTypes, Component, type ComponentClass, type ComponentLifecycle, type ComponentPropsWithRef, type ComponentState, type ComponentType, type CompositionEvent$1 as CompositionEvent, type CompositionEventHandler, ComputedImpl, type ComputedValues, type Context$1 as Context, type CurrentListener, type DependencyList, type Dispatch$1 as Dispatch, type DispatchVoid, type DragEvent$1 as DragEvent, type DragEventHandler, type EffectCallBack, type EventHandler, type ExoticComponent, type FC, type FnType, type FocusEvent$1 as FocusEvent, type FocusEventHandler, type FormEvent, type FormEventHandler, TYPE_FORWARD_REF as ForwardRef, type ForwardRefRenderFunc, TYPE_FRAGMENT as Fragment, type FunctionComponent, type IObserver, type IfAny, type Instance, type InulaElement, type InulaFragment, type InulaNode, type InulaPortal, type InulaProfiler, type InvalidEvent, type IterableTypes, type JSXElementConstructor, type KVObject, type Key, type KeyType, type KeyboardEvent$1 as KeyboardEvent, type KeyboardEventHandler, type LazyComponent, type Listener, type Listeners, type MapTypes, type MaybeRef, TYPE_MEMO as Memo, type MemoComponent, type ModifierKey, type MouseEvent$1 as MouseEvent, type MouseEventHandler, type Mutation, type ObjectType, type PlannedAction, type PointerEvent$1 as PointerEvent, type PointerEventHandler, TYPE_PROFILER as Profiler, type PropsOmitRef, type PropsWithRef, PureComponent, type QueuedStoreActions, type Raw, RawSymbol, type ReactiveRet, type Reducer$1 as Reducer, type ReducerAction, type ReducerState, type ReducerStateVoid, type ReducerVoid, type Ref, type RefAttributes, type RefType, type SetTypes, type StoreActions, type StoreComputed, type StoreConfig, type StoreObj, TYPE_STRICT_MODE as StrictMode, TYPE_SUSPENSE as Suspense, type ToRef, type ToRefs, type TouchEvent$1 as TouchEvent, type TouchEventHandler, type TransitionEvent$1 as TransitionEvent, type TransitionEventHandler, type UIEvent$1 as UIEvent, type UIEventHandler, type UnwrapRef, type UserActions, type UserComputedValues, VNode, type WatchCallback, type WatchFn, type WatchHandler, type WheelEvent$1 as WheelEvent, type WheelEventHandler, act, clearStore, cloneElement, createContext, createElement, createPortal, createRef, createRootElement as createRoot, createStore$1 as createStore, Horizon as default, findDOMNode, syncUpdates as flushSync, forwardRef, isContextConsumer, isContextProvider, isElement, isForwardRef, isFragment, isLazy, isMemo, isPortal, isValidElement, isValidElementType, lazy, memo, reduxAdapter, executeRender as render, destroy as unmountComponentAtNode, asyncUpdates as unstable_batchedUpdates, useCallback, useContext, useDebugValue, useEffect, useImperativeHandle, useLayoutEffect, useMemo, useReducer, useRef, useState, useStore$1 as useStore, vueReactive, watch };
