...
const UserProfile = ({
  ...
  self.updateProp = (propName: string, newValue: Value) => {
    switch (propName) {
      case 'name': self.runWithCache('j4toaf', () => name = newValue, [name]); break;
      case 'age': self.runWithCache('24fjae', () => age = newValue, [age]); break;
      case 'favouriteColors': self.runWithCache('vja03k', () => favouriteColors = newValue, [favouriteColors]); break;
      case 'isAvailable': self.runWithCache('agj2io', () => isAvailable = newValue, [isAvailable]); break;
    }
  }
  ...
  self.body = () => {
    ...
    setHTMLProp($node3, 'textContent', () => `My favourite colors are ${favouriteColors.join(', ')}`, [favouriteColors]);
    ...
    return [
      ...
      (dirty) => {
        ...
        if (dirty & 0b0100) setHTMLProp($node3, 'textContent', () => `My favourite colors are ${favouriteColors.join(', ')}`, [favouriteColors]);
        if (dirty & 0b1000) setHTMLProp($node4, 'textContent', () => `I am ${isAvailable ? 'available' : 'not available'}`, [isAvailable]);
      },
    ]
  }
