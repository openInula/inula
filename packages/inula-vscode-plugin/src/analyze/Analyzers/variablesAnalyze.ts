  ...
  return {
    VariableDeclaration(path: NodePath<t.VariableDeclaration>, ctx) {
      ...
      declarations.forEach(declaration => {
        ...
        if (t.isLVal(id.node)) {
          assertIdOrDeconstruct(id as NodePath<t.LVal>, 'Invalid Variable type when adding variable: ' + id.type);
        } else {
          throw new CompilerError('Unsupported pattern for variable declaration', id.node.loc);
        }
