...
function Prompt({
  ...
  const minHeight = initEditorHeight || 100;
  ...
  return (
    <div
      className={cn(
        className,
        'relative rounded-xl bg-gradient-to-r border-gray-100 border-4',
      )}
    >
      <div className="rounded-xl bg-background-section-burn">
        ...
        <PromptEditorHeightResizeWrap
          className="min-h-[100px] rounded-t-xl bg-background-default px-4 pt-2 text-sm text-text-secondary"
          height={editorHeight}
          minHeight={minHeight}
      ...
      <Drawer title="系统提示词" placement="right" onClose={onClose} open={open} width={800}>
        <Space direction="vertical" style={{ width: '100%' }}>
          <pre className='whitespace-pre-wrap break-words'>{dslSystemPrompt}</pre>
        </Space>
      </Drawer>
    </div>
  );
