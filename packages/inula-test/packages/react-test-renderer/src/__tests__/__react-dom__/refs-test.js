...
let ReactTestUtils = require('react-test-renderer/test-utils');
...
describe('ref swapping', () => {
  ...
  xit('provides an error for invalid refs', () => {
    expect(() => {
      ReactTestUtils.renderIntoDocument(<div ref={10} />);
    }).toThrow(
      'attribute ref should be: function / string /object returned by horizon.createRef() / null.',
    );
    expect(() => {
