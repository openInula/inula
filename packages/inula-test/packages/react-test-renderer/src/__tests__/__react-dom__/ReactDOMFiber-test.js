...
describe('ReactDOMFiber', () => {
  ...
  const expectSVG = {ref: el => {
    return svgEls.push(el);
  }};
  const expectHTML = {ref: el => {
    return htmlEls.push(el);
  }};
  const expectMath = {ref: el => {
    return mathEls.push(el);
  }};
  ...
  // portal lazy
  xit('listens to events that do not exist in the Portal subtree', () => {
  ...
  xit('should warn for non-functional event listeners', () => {
  ...
  xit('should warn with a special message for `false` event listeners', () => {
    class Example extends React.Component {
      render() {
    ...
    expect(() => ReactDOM.render(<Example />, container)).toErrorDev(
      '`onClick` should be a function, but it’s a `boolean` type.'
    );
  });
