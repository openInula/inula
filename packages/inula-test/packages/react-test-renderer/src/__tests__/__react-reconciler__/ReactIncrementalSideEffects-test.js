...
describe('ReactIncrementalSideEffects', () => {
  ...
  // Concurrent
  xit('does not update child nodes if a flush is aborted', () => {
    function Bar(props) {
    ...
    ReactNoop.render(<Foo text="Hello" />);
    expect(Scheduler).toHaveYielded(['Foo', 'Bar', 'Bar', 'Bar']);
    expect(ReactNoop.getChildren()).toEqual([
  ...
  // Concurrent
  xit('can update a completed tree before it has a chance to commit', () => {
    function Foo(props) {
      Scheduler.unstable_yieldValue('Foo');
