...
describe('ReactIncrementalErrorHandling', () => {
  ...
  // Concurrent
  xit('retries at a lower priority if there\'s additional pending work', () => {
    function App(props) {
      if (props.isBroken) {
  ...
  // Concurrent
  xit('retries one more time before handling error', () => {
    function BadRender({unused}) {
  ...
  // Concurrent
  xit('retries one more time if an error occurs during a render that expires midway through the tree', () => {
  ...
  xit('catches render error in a boundary during batched mounting', () => {
    class ErrorBoundary extends React.Component {
      state = {error: null};
  ...
  xit('propagates an error from a noop error boundary during batched mounting', () => {
    class RethrowErrorBoundary extends React.Component {
      componentDidCatch(error) {
        Scheduler.unstable_yieldValue('RethrowErrorBoundary componentDidCatch');
        throw error;
  ...
  it('can schedule updates after uncaught error in render on update', () => {
    ...
    expect(Scheduler).toHaveYielded(['BrokenRender']);
  ...
  it('catches reconciler errors in a boundary during mounting', () => {
    ...
    expect(ReactNoop.getChildren()).toEqual([
      span(
        'Component type is invalid, got: undefined',
      ),
    ]);
  });
  ...
  it('does not interrupt unmounting if detaching a ref throws', () => {
    ...
    ReactNoop.render(<Foo />);
    expect(Scheduler).toHaveYielded(['barRef attach']);
  ...
  it('provides component stack to the error boundary with componentDidCatch', () => {
    ...
    expect(Scheduler).toHaveYielded(['render error message']);
  });
  ...
  // Concurrent
  xit('uncaught errors should be discarded if the render is aborted', async () => {
    ...
    await ReactNoop.act(async () => {
      ...
      // recover gracefully.
      ReactNoop.render('Everything is fine.');
    });

    // Should finish without throwing.
    expect(ReactNoop).toMatchRenderedOutput('Everything is fine.');
  });

  // Concurrent
  xit('uncaught errors are discarded if the render is aborted, case 2', async () => {
