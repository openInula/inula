...
describe('ReactChildren', () => {
  ...
  it('should support children has the same keys', () => {
    const callback = jest.fn().mockImplementation(function (kid, index) {
      const key = '.' + index;
      // 返回值和getItemKey返回值一致
      return <span key={key} />;
  ...
  xit('should be called for each child in an iterable with keys', () => {
  ...
  it('should be called for each child', () => {
    ...
    ]).toEqual(['.$keyZero.$giraffe', '.$keyTwo', '.3', '.$keyFour']);
  ...
  it('should be called for each child in nested structure', () => {
    ...
    expect(mappedChildren[0]).toEqual(<div key=".0.0.$keyZero.$giraffe" />);
  ...
  it('should retain key across two mappings', () => {
    ...
    const expectedForcedKeys = ['.$keyZero.$giraffe', '.$keyOne'];
  ...
  it('should throw on object', () => {
    expect(function () {
      ...
      'Object is invalid as a Inula child. '
    );
  });

  xit('should throw on regex', () => {
  ...
  describe('with fragments enabled', () => {
    ...
    it('warns for keys for arrays at the top level', () => {
      expect(() => ReactTestUtils.renderIntoDocument([<div />, <div />]));
    });
  });
});
