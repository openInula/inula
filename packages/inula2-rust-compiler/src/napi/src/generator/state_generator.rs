...
use crate::types::{StateStmt, DerivedStmt, IRStmt, DerivedSource};
...
impl StateGenerator {
    ...
    /// 生成派生状态语句
    pub fn generate_derived(&self, stmt: &DerivedStmt, ctx: &GeneratorContext) -> CompilerResult<Vec<Stmt>> {
        // 对应原版的 derived 函数
    ...
    fn create_derived_declaration(&self, stmt: &DerivedStmt) -> CompilerResult<Stmt> {
        let var_decl = VarDecl {
            ...
            decls: vec![VarDeclarator {
                span: swc_common::Span::new(swc_common::BytePos(0), swc_common::BytePos(0)),
                name: Pat::Ident(BindingIdent {
                    id: Ident::new(stmt.name.clone().into(), swc_common::Span::new(swc_common::BytePos(0), swc_common::BytePos(0)), swc_common::SyntaxContext::empty()),
                    type_ann: None,
                }),
                init: None,
                definite: false,
    ...
    /// 创建更新调用
    fn create_update_call(&self, stmt: &DerivedStmt, ctx: &GeneratorContext) -> CompilerResult<Stmt> {
        // 根据原版逻辑创建更新调用
    ...
    /// 创建 Hook 更新调用
    fn create_hook_update_call(&self, stmt: &DerivedStmt, ctx: &GeneratorContext) -> CompilerResult<Expr> {
        // 对应原版的 Hook 更新逻辑
        let value_ident = Ident::new("$$value".into(), swc_common::Span::new(swc_common::BytePos(0), swc_common::BytePos(0)), swc_common::SyntaxContext::empty());
        
        let update_fn = Expr::Arrow(ArrowExpr {
            span: swc_common::Span::new(swc_common::BytePos(0), swc_common::BytePos(0)),
            params: vec![
                Pat::Ident(BindingIdent { id: value_ident.clone(), type_ann: None })
            ],
            body: Box::new(BlockStmtOrExpr::Expr(Box::new(Expr::Call(CallExpr {
                ...
                callee: Callee::Expr(Box::new(Expr::Member(MemberExpr {
                    ...
                    prop: MemberProp::Ident(Ident::new("wave".into(), swc_common::Span::new(swc_common::BytePos(0), swc_common::BytePos(0)), swc_common::SyntaxContext::empty()).into()),
                }))),
                args: vec![
                    ExprOrSpread {
                        spread: None,
                        expr: Box::new(Expr::Paren(ParenExpr {
                            ...
                            expr: Box::new(Expr::Assign(AssignExpr {
                                span: swc_common::Span::new(swc_common::BytePos(0), swc_common::BytePos(0)),
                                left: AssignTarget::Simple(SimpleAssignTarget::Ident(BindingIdent { id: Ident::new(
                                    stmt.name.clone().into(),
                                    swc_common::Span::new(swc_common::BytePos(0), swc_common::BytePos(0)),
                                    swc_common::SyntaxContext::empty(),
                                ), type_ann: None })),
        ...
        let use_hook_call = Expr::Call(CallExpr {
            ...
            callee: Callee::Expr(Box::new(Expr::Member(MemberExpr {
                span: swc_common::Span::new(swc_common::BytePos(0), swc_common::BytePos(0)),
                obj: Box::new(Expr::Ident(ctx.self_id.clone())),
                prop: MemberProp::Ident(Ident::new("useHook".into(), swc_common::Span::new(swc_common::BytePos(0), swc_common::BytePos(0)), swc_common::SyntaxContext::empty()).into()),
            }))),
            args: vec![
                ExprOrSpread {
                    spread: None,
    ...
    /// 创建派生状态调用
    fn create_derive_state_call(&self, stmt: &DerivedStmt, ctx: &GeneratorContext) -> CompilerResult<Expr> {
        ...
        let derive_state_call = Expr::Call(CallExpr {
            ...
            callee: Callee::Expr(Box::new(Expr::Member(MemberExpr {
                span: swc_common::Span::new(swc_common::BytePos(0), swc_common::BytePos(0)),
                obj: Box::new(Expr::Ident(ctx.self_id.clone())),
                prop: MemberProp::Ident(Ident::new("deriveState".into(), swc_common::Span::new(swc_common::BytePos(0), swc_common::BytePos(0)), swc_common::SyntaxContext::empty()).into()),
            }))),
            args: vec![
                ExprOrSpread {
                    spread: None,
                ...
                ExprOrSpread {
                    spread: None,
                    expr: Box::new(Expr::Lit(Lit::Num(Number {
                        value: (ctx.get_react_bits)(stmt.dependency.as_ref().map(|d| d.dep_id_bitmap).unwrap_or(0)) as f64,
                        span: swc_common::Span::new(swc_common::BytePos(0), swc_common::BytePos(0)),
                        raw: None,
                    }))),
                },
