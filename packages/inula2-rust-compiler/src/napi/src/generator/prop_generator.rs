...
use crate::types::{SinglePropStmt, RestPropStmt, WholePropStmt, UseContextStmt, IRStmt};
...
impl PropGenerator {
    /// 生成单个属性语句
    pub fn generate_single_prop(&self, stmt: &SinglePropStmt, ctx: &GeneratorContext) -> CompilerResult<Stmt> {
    ...
    /// 生成剩余属性语句
    pub fn generate_rest_prop(&self, stmt: &RestPropStmt, ctx: &GeneratorContext) -> CompilerResult<Stmt> {
        ...
        Ok(Stmt::Expr(ExprStmt { span: swc_common::Span::new(swc_common::BytePos(0), swc_common::BytePos(0)), expr: Box::new(call) }))
    }

    /// 生成整体属性语句
    pub fn generate_whole_prop(&self, stmt: &WholePropStmt, ctx: &GeneratorContext) -> CompilerResult<Stmt> {
        let var_decl = VarDecl {
            span: swc_common::Span::new(swc_common::BytePos(0), swc_common::BytePos(0)),
            ctxt: swc_common::SyntaxContext::empty(),
            kind: VarDeclKind::Let,
    ...
    /// 生成 useContext 语句
    pub fn generate_use_context(&self, stmt: &UseContextStmt, ctx: &GeneratorContext) -> CompilerResult<Stmt> {
