...
use crate::types::{RawStmt, IRStmt};
...
impl RawStmtGenerator {
    /// 生成原始语句
    pub fn generate_raw_stmt(&self, stmt: &RawStmt, ctx: &GeneratorContext) -> CompilerResult<Stmt> {
        self.parse_raw_statement(&stmt.value)
    }

    fn parse_raw_statement(&self, stmt_str: &str) -> CompilerResult<Stmt> {
        if stmt_str.starts_with("const ") || stmt_str.starts_with("let ") || stmt_str.starts_with("var ") {
    ...
    fn parse_variable_declaration(&self, stmt_str: &str) -> CompilerResult<Stmt> {
        let kind = if stmt_str.starts_with("const ") { VarDeclKind::Const } else if stmt_str.starts_with("let ") { VarDeclKind::Let } else { VarDeclKind::Var };
    ...
    fn extract_variable_name(&self, stmt_str: &str) -> String {
        ...
        if parts.len() > 1 { parts[1].to_string() } else { "unknown".to_string() }
    }
}
