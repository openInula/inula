...
const FIXTURE_DIR = path.resolve(process.cwd(), 'tests/suspense-lazy');
...
async function transformSource(source, filePath) {
  const res = await runJscodeshiftTransforms({ filePath, source, onlyRules: ['core/suspense-lazy'] });
  return res.source;
}

describe('Suspense & lazy fixtures (before -> after)', () => {
