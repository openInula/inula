...
const FIXTURE_DIR = path.resolve(process.cwd(), 'tests/context');
...
async function transformSource(source, filePath) {
  const res = await runJscodeshiftTransforms({ filePath, source, onlyRules: ['core/context','core/imports'] });
  return res.source;
}

describe('Context fixtures (before -> after)', () => {
