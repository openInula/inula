...
const FIXTURE_DIR = path.resolve(process.cwd(), 'tests/components');
...
async function transformSource(source, filePath) {
  const res = await runJscodeshiftTransforms({ filePath, source, onlyRules: ['core/components'] });
  return res.source;
}

describe('Components fixtures (before -> after)', () => {
