...
const FIXTURE_DIR = path.resolve(process.cwd(), 'tests/dynamic');
...
async function transformSource(source, filePath) {
  const res = await runJscodeshiftTransforms({ filePath, source, onlyRules: ['core/dynamic'] });
  return res.source;
}

describe('Dynamic fixtures (before -> after)', () => {
