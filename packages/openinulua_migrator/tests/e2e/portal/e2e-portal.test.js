...
const FIXTURE_DIR = path.resolve(process.cwd(), 'tests/portal');
...
async function transformSource(source, filePath) {
  const res = await runJscodeshiftTransforms({ filePath, source, onlyRules: ['core/portal'] });
  return res.source;
}

describe('Portal fixtures (before -> after)', () => {
