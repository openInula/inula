...
const FIXTURE_DIR = path.resolve(process.cwd(), 'tests/error-boundary');
...
async function transformSource(source, filePath) {
  const res = await runJscodeshiftTransforms({ filePath, source, onlyRules: ['core/error-boundary'] });
  return res.source;
}

describe('ErrorBoundary fixtures (before -> after)', () => {
