...
const FIXTURE_DIR = path.resolve(process.cwd(), 'tests/props');
...
async function transformSource(source, filePath) {
  // Test props in isolation; other transforms should not change output
  const res = await runJscodeshiftTransforms({ filePath, source, onlyRules: ['core/props'] });
  return res.source;
}

describe('Props fixtures (before -> after)', () => {
